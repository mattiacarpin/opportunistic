...
int TdMtType0AllocationRbg[4] = { 10,       // RGB size 1
		26,       // RGB size 2
		63,       // RGB size 3
		110       // RGB size 4
...
TdMtSchedulerMemberCschedSapProvider::TdMtSchedulerMemberCschedSapProvider(
		TdMtFfMacScheduler* scheduler) :
						m_scheduler(scheduler) {
}

void TdMtSchedulerMemberCschedSapProvider::CschedCellConfigReq(
		const struct CschedCellConfigReqParameters& params) {
	m_scheduler->DoCschedCellConfigReq(params);
}

void TdMtSchedulerMemberCschedSapProvider::CschedUeConfigReq(
		const struct CschedUeConfigReqParameters& params) {
	m_scheduler->DoCschedUeConfigReq(params);
}

void TdMtSchedulerMemberCschedSapProvider::CschedLcConfigReq(
		const struct CschedLcConfigReqParameters& params) {
	m_scheduler->DoCschedLcConfigReq(params);
}

void TdMtSchedulerMemberCschedSapProvider::CschedLcReleaseReq(
		const struct CschedLcReleaseReqParameters& params) {
	m_scheduler->DoCschedLcReleaseReq(params);
}

void TdMtSchedulerMemberCschedSapProvider::CschedUeReleaseReq(
		const struct CschedUeReleaseReqParameters& params) {
...
void TdMtSchedulerMemberSchedSapProvider::SchedDlRlcBufferReq(
		const struct SchedDlRlcBufferReqParameters& params) {
	m_scheduler->DoSchedDlRlcBufferReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedDlPagingBufferReq(
		const struct SchedDlPagingBufferReqParameters& params) {
...
void TdMtSchedulerMemberSchedSapProvider::SchedDlTriggerReq(
		const struct SchedDlTriggerReqParameters& params) {
	m_scheduler->DoSchedDlTriggerReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedDlRachInfoReq(
		const struct SchedDlRachInfoReqParameters& params) {
	m_scheduler->DoSchedDlRachInfoReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedDlCqiInfoReq(
		const struct SchedDlCqiInfoReqParameters& params) {
	m_scheduler->DoSchedDlCqiInfoReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedUlTriggerReq(
		const struct SchedUlTriggerReqParameters& params) {
	m_scheduler->DoSchedUlTriggerReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedUlNoiseInterferenceReq(
		const struct SchedUlNoiseInterferenceReqParameters& params) {
	m_scheduler->DoSchedUlNoiseInterferenceReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedUlSrInfoReq(
		const struct SchedUlSrInfoReqParameters& params) {
	m_scheduler->DoSchedUlSrInfoReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedUlMacCtrlInfoReq(
		const struct SchedUlMacCtrlInfoReqParameters& params) {
	m_scheduler->DoSchedUlMacCtrlInfoReq(params);
}

void TdMtSchedulerMemberSchedSapProvider::SchedUlCqiInfoReq(
		const struct SchedUlCqiInfoReqParameters& params) {
	m_scheduler->DoSchedUlCqiInfoReq(params);
}

TdMtFfMacScheduler::TdMtFfMacScheduler() :
					m_cschedSapUser(0), m_schedSapUser(0), m_nextRntiUl(0) {
	...
	//TODO: Here we have the constructor
	mapFull=false;

	// Look for the numberOfUsers in the file!
	std::ifstream nUES ("numberOfNodes.txt");
	if (nUES.is_open())
	{	std::string line;
		while ( std::getline (nUES,line) )
		{
...
int TdMtFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
	std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
	int lcActive = 0;
...
uint8_t TdMtFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
void TdMtFfMacScheduler::DoSchedDlTriggerReq(
		...
		if (wbCqi != 0)
		{
			...
			if (LcActivePerFlow(*it) > 0)
			{
				...
				for (uint8_t k = 0; k < nLayer; k++)
				{
					...
					achievableRate += ((m_amc->GetTbSizeFromMcs(mcs, rbgSize)
							/ 8) / 0.001); // = TB size / TTI
				...
				if(mapFull)
				{
					...
					int currentRNTI=(*it);		//This is the current RNTI

					int IMSI=getIMSI(currentRNTI);
					if (IMSI==0)
					{
						std::cout << "Error, the RNTI "<<currentRNTI<<" has not been matched"<<std::endl;
					}
					else
					{
						alpha=alphas[IMSI-1];
	...
	{
		// assign all free RBGs to this UE (because we are working in a TD approach)!!
	...
	while (itMap != allocationMap.end()) {
		...
		BuildDataListElement_s newEl;
		newEl.m_rnti = (*itMap).first;		//Get the RNTI of the UE
...
void TdMtFfMacScheduler::RefreshDlCqiMaps(void) {
...
void TdMtFfMacScheduler::RefreshUlCqiMaps(void) {
	// refresh UL CQI  Map
...
bool TdMtFfMacScheduler::refreshMap(int time)
{
	//Open the file, see if we have all the infos, update the RNTI_IMSI_Map and return the result

	std::map <int, int> IMSI_Temp;

	std::ifstream FileMap3 ("RNTI_IMSI_MAP_2.txt");
	if (FileMap3.is_open())
	{	std::string line;
		while ( std::getline (FileMap3,line) )
		{
			std::istringstream iss(line);
			int RNTI;
			iss >> RNTI;
			int IMSI;
			iss >> IMSI;
			IMSI_Temp.insert(std::pair<int,int>(RNTI,IMSI));
		}
		FileMap3.close();
	}

	uint16_t n=(uint16_t)(numberOfNodes);
	if(IMSI_Temp.size()==n)
	{
		RNTI_IMSI_Map=IMSI_Temp;
		std::cout<<"We have received all the information to fill the RNTI - IMSI map at "<<time*10<<" ms."<<std::endl;
		std::map<int,int>::iterator it = RNTI_IMSI_Map.begin();

		std::cout << "RNTI" << " <=> " << "IMSI" << '\n';
		for (it=RNTI_IMSI_Map.begin(); it!=RNTI_IMSI_Map.end(); ++it)
...
/*double TdMtFfMacScheduler::getAlphaValue(int IMSI)
{
	if (IMSI>numberOfNodes)
	{
		std::cout<<"ERROR: We cannot get alpha for IMSI="<<IMSI<<" ("<<numberOfNodes<<" UEs)"<<std::endl;
		return 1;
	}
	return alphas[IMSI-1];
}*/

int TdMtFfMacScheduler::getIMSI(int RNTI)
{
	std::map<int,int>::iterator it;
	it=RNTI_IMSI_Map.find(RNTI);		//Find the RNTI in the map

	return (it == RNTI_IMSI_Map.end())?0:(it->second);
}
