...
void TdMtFfMacScheduler::DoSchedDlTriggerReq(
	...
	if ((params.m_sfnSf >> 4)==4)
	{
		//std::cout<<"Sono entrato a regime"<<std::endl;
	}
	...
	for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++) {
		...
		if (wbCqi != 0)
		{
			...
			if (LcActivePerFlow(*it) > 0)
			{
				...
				//TODO
				//Do we have the association between IMSI and RNTI? Check!
				double alpha=1;
				if(mapFull)
				{
					...
					it=RNTI_IMSI_Map.find(currentRNTI);		//Find the RNTI in the map

					if(it == RNTI_IMSI_Map.end())
					{
						std::cout << "Error, the RNTI "<<currentRNTI<<" has not been matched"<<std::endl;
					}
					else
					{
						int IMSI=it->second;					//Get the corresponding IMSI
						alpha=getAlphaValue(IMSI);
					}
				}
				else
				{
					std::cout << "Frame=" << (params.m_sfnSf >> 4) << ", Subframe=" << (0xF & params.m_sfnSf)<<" not received all infos."<<std::endl;
					//We don't know anything, open the file and see if the infos are ready to be used
					mapFull=refreshMap();
				}
...
bool TdMtFfMacScheduler::refreshMap()
{
	...
	int totalUEs=m_flowStatsDl.size();
	if (lineCount==0)
	{return false;}
	else if (lineCount==totalUEs)
	{
		//Fill the map with useful information taken from the file
		std::ifstream FileMap2 ("RNTI_IMSI_MAP.txt");
		if (FileMap2.is_open())
		{
			while ( std::getline (FileMap2,line) )
			{
...
double TdMtFfMacScheduler::getAlphaValue(int IMSI){return 1;}
}
